...
# ──── AI Stats ────
def get_ai_stats():
    """Read AI stats from live_accuracy.json (updated every 5min)"""
    if not os.path.exists(AI_LOG):
        return None
    
    acc_json = os.path.join(DATA_DIR, '..', 'live_accuracy.json')
    ...
    # Read accuracy from JSON file (generated by save_accuracy.py every 5min)
    live_acc = None
    high_conf_acc = None
    signals_checked = 0
    if os.path.exists(acc_json):
        try:
            ...
            live_acc = acc_data.get('live_accuracy')
            high_conf_acc = acc_data.get('high_conf_accuracy')
            signals_checked = acc_data.get('signals_checked', 0)
        except:
            pass
