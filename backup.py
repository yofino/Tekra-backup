...
Retention: semua backup > 7 hari otomatis dihapus
"""
...
PORTALS = [
    {"name": "portal-pusat",   "host": "103.127.139.207", "db": "portal",  "user": "portal",  "pass": "portal",  "ssh_user": "keanuvps", "ssh_key": "/root/.ssh/nocita_key", "ssh_pass": "k34Nu335577"},
    {"name": "portal-dago",    "host": "103.127.139.207", "db": "utara",   "user": "utara",   "pass": "utara",   "ssh_user": "keanuvps", "ssh_key": "/root/.ssh/nocita_key", "ssh_pass": "k34Nu335577"},
    {"name": "portal-katapang","host": "103.127.139.207", "db": "selatan", "user": "selatan", "pass": "selatan", "ssh_user": "keanuvps", "ssh_key": "/root/.ssh/nocita_key", "ssh_pass": "k34Nu335577"},
]

AGENT_WORKSPACES = [
    {"name": "Siti", "host": "103.127.139.207", "ssh_user": "keanuvps", "ssh_key": "/root/.ssh/nocita_key", "ssh_pass": "k34Nu335577", "workspace": "/opt/openclaw/data/workspace", "sudo": True},
]
...
def cleanup_old_backups(keep_days=7):
    """Hapus backup > 7 hari (databases, workspace, workspace-agents)"""
    cutoff = datetime.date.today() - datetime.timedelta(days=keep_days)
    targets = [
        (DATA_DIR / "databases", "*.sql.gz"),
        (DATA_DIR / "workspace", "*.tar.gz"),
        (DATA_DIR / "workspace-agents", "*.tar.gz"),
    ]
    for folder, pattern in targets:
        ...
        for f in folder.glob(pattern):
            try:
                ...
                if file_date < cutoff:
                    f.unlink()
                    log(f"Cleaned up old backup: {f.name}")
            except Exception:
                pass
